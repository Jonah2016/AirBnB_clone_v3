...
class TestUserViews(unittest.TestCase):
    ...
    def tearDown(self):
        """Tear down context """
        if (os.getenv("HBNB_TYPE_STORAGE") == "db"):
            storage.delete(self.akingbeni_usr_obj)
            storage.delete(self.david_usr_obj)
            storage.save()
        else:
            os.remove("file.json")
    ...
    def setUp(self):
        """ Set up context """
        try:
            self.akingbeni_usr_obj = User(**{
                "email": "xyz@mail.com",
                "password": "xyz",
                "first_name": "akingbeni",
                "last_name": "akin"
            })
            #  setattr(self.akingbeni_usr_obj, "name", "Akingbeni")
            self.david_usr_obj = User(**{
                "email": "abc@mail.com",
                "password": "123",
                "first_name": "david",
                "last_name": "dave"
            })
            #  setattr(self.david_usr_obj, "name", "David")
            storage.new(self.akingbeni_usr_obj)
            storage.new(self.david_usr_obj)
            storage.save()
        except (SQLAlchemyError):
            storage.delete(self.akingbeni_usr_obj)
            storage.delete(self.david_usr_obj)
